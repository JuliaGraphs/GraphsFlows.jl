@testset "Dinic" begin
    ...
    for fg in (flow_graph, Graphs.DiGraph{UInt8}(flow_graph), Graphs.DiGraph{Int16}(flow_graph))
      ...
      @test @inferred(GraphsFlows.dinic_impl(residual_graph, 1, 8, GraphsFlows.DefaultCapacity(residual_graph)))[1] == 3

      # Test with capacity matrix
      @test @inferred(GraphsFlows.dinic_impl(residual_graph, 1, 8, capacity_matrix))[1] == 28
      ...
      function test_blocking_flow(residual_graph, source, target, capacity_matrix, flow_matrix)
          ...
          @test @inferred(GraphsFlows.blocking_flow(h, source, target, capacity_matrix, flow_matrix)) == 0
          ...
          capacity_matrix_ = vcat(hcat(capacity_matrix, zeros(Int, Graphs.nv(residual_graph))), zeros(Int, 1, Graphs.nv(residual_graph) + 1))
          flow_graph_  = vcat(hcat(flow_matrix, zeros(Int, Graphs.nv(residual_graph))), zeros(Int, 1, Graphs.nv(residual_graph) + 1))

          @test @inferred(GraphsFlows.blocking_flow(h, source, target, capacity_matrix_, flow_graph_)) > 0

          #test with connected graph
          @test @inferred(GraphsFlows.blocking_flow(residual_graph, source, target, capacity_matrix, flow_matrix)) > 0
      end
